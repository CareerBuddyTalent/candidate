<template>
  <main>
    <div v-for="detail in details" :key="detail">
      <div
        class="border border-brand-black/5 bg-brand-light/40 cursor-pointer rounded-xl lg:p-6 p-3 flex flex-col lg:flex-row justify-between mb-4 lg:space-x-32"
      ...
      >
        <div class="flex items-center">
          ...
          <div class="flex flex-col">
            <p class="font-albertBold lg:text-xl text-base text-brand-black text-justify">
              {{ detail.title }}
            </p>
        ...
        <div class="lg:flex justify-between grid items-center" :class="[viewType == 'application' ? 'grow grid-rows-3 grid-flow-col gap-y-4' : '']">
          ...
          >
            <p class="font-normal lg:text-lg text-sm text-gray-500 capitalize">
              {{ detail.type.split('_').join('-') }} | {{ detail.work_type.split('_').join('-') }}
            </p>
            <p class="text-2xl text-[#12080F]/50 lg:hidden mb-2.5">.</p>
            <p class="lg:text-base text-sm font-albertSemiBold text-brand-black/90">
              {{ detail.salary_min.formatted }} - {{ detail.salary_max.formatted }}
            </p>
          </div>
